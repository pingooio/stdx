...
#[deprecated(since = "0.21.0", note = "Use Engine::decode")]
#[cfg(any(feature = "alloc", test))]
pub fn decode_engine<E: Engine, T: AsRef<[u8]>>(input: T, engine: &E) -> Result<Vec<u8>, DecodeError> {
    engine.decode(input)
}
...
mod tests {
    ...
    fn decode_into_nonempty_vec_doesnt_clobber_existing_prefix() {
        ...
        for _ in 0..10_000 {
            ...
            engine.decode_vec(&encoded_data, &mut decoded_with_prefix).unwrap();
            // also decode into the empty buf
            engine.decode_vec(&encoded_data, &mut decoded_without_prefix).unwrap();
    ...
    fn decode_slice_output_length_errors() {
        for num_quads in 1..100 {
            ...
            vec.push(0);
            // now it works
            assert_eq!(num_quads * 3, STANDARD.decode_slice(&input, &mut vec).unwrap());
        }
    }

    fn do_decode_slice_doesnt_clobber_existing_prefix_or_suffix<F: Fn(&GeneralPurpose, &[u8], &mut [u8]) -> usize>(
        call_decode: F,
    ) {
        let mut orig_data = Vec::new();
        let mut encoded_data = String::new();
        ...
        for _ in 0..10_000 {
            ...
            // decode into the non-empty buf
            let decode_bytes_written = call_decode(&engine, encoded_data.as_bytes(), &mut decode_buf[offset..]);
