...
#[cfg(any(feature = "alloc", test))]
pub fn encode_engine_string<E: Engine, T: AsRef<[u8]>>(input: T, output_buf: &mut String, engine: &E) {
...
mod tests {
    ...
    fn encode_engine_string_into_nonempty_buffer_doesnt_clobber_prefix() {
        ...
        for _ in 0..10_000 {
            ...
            assert_encode_sanity(&encoded_data_no_prefix, engine.config().encode_padding(), input_len);
            ...
            engine.decode_vec(&encoded_data_no_prefix, &mut decoded).unwrap();
    ...
    fn encode_engine_slice_into_nonempty_buffer_doesnt_clobber_suffix() {
        ...
        for _ in 0..10_000 {
            ...
            assert_eq!(encoded_size, engine.encode_slice(&orig_data, &mut encoded_data).unwrap());
            ...
            engine.decode_vec(&encoded_data[0..encoded_size], &mut decoded).unwrap();
            assert_eq!(orig_data, decoded);
        }
    }
    ...
    fn assert_encoded_length<E: Engine>(input_len: usize, enc_len: usize, engine: &E, padded: bool) {
