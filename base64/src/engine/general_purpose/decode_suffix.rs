...
pub(crate) fn decode_suffix(
    ...
    for (leftover_index, &b) in input[input_index..].iter().enumerate() {
        ...
        // erroneous padding.
        if padding_bytes_count > 0 {
            return Err(DecodeError::InvalidByte(input_index + first_padding_offset, PAD_BYTE).into());
        }
    ...
    if !decode_allow_trailing_bits && (leftover_num & mask) != 0 {
        ...
        return Err(DecodeError::InvalidLastSymbol(input_index + morsels_in_leftover - 1, last_symbol).into());
    }
