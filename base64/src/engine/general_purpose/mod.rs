...
impl super::Engine for GeneralPurpose {
    ...
    fn internal_encode(&self, input: &[u8], output: &mut [u8]) -> usize {
        ...
        while input_index < start_of_rem {
            ...
            output_chunk[1] =
                self.encode_table[((input_chunk[0] << 4 | input_chunk[1] >> 4) & LOW_SIX_BITS_U8) as usize];
            output_chunk[2] =
                self.encode_table[((input_chunk[1] << 2 | input_chunk[2] >> 6) & LOW_SIX_BITS_U8) as usize];
        ...
        if rem == 2 {
            ...
            output[output_index + 1] = self.encode_table
