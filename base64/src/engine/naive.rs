...
impl Engine for Naive {
    ...
    fn internal_encode(&self, input: &[u8], output: &mut [u8]) -> usize {
        ...
        if let Some(last_complete_chunk_index) = complete_chunk_len.checked_sub(Self::ENCODE_INPUT_CHUNK_SIZE) {
            while input_index <= last_complete_chunk_index {
                ...
                output[output_index + 1] = self.encode_table[chunk_int.shr(12_u8).bitand(LOW_SIX_BITS) as usize];
                output[output_index + 2] = self.encode_table[chunk_int.shr(6_u8).bitand(LOW_SIX_BITS) as usize];
                output[output_index + 3] = self.encode_table[chunk_int.bitand(LOW_SIX_BITS) as usize];
        ...
        if rem == 2 {
            ...
            output[output_index + 1] =
                self.encode_table[(chunk[0].shl(4_u8).bitor(chunk[1].shr(4_u8)) as u32).bitand(LOW_SIX_BITS) as usize];
    ...
    fn internal_decode(
        ...
        for (chunk_index, chunk) in input[..complete_nonterminal_quads_len].chunks_exact(4).enumerate() {
            ...
            let decoded_int: u32 = self.decode_byte_into_u32(input_index, chunk[0])?.shl(18)
                | self.decode_byte_into_u32(input_index + 1, chunk[1])?.shl(12)
