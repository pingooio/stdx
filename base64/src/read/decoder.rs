...
impl<'e, E: Engine, R: io::Read> DecoderReader<'e, E, R> {
    ...
    fn flush_decoded_buf(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        buf[..copy_len]
            .copy_from_slice(&self.decoded_chunk_buffer[self.decoded_offset..self.decoded_offset + copy_len]);
    ...
    fn decode_to_buf(&mut self, b64_len_to_decode: usize, buf: &mut [u8]) -> io::Result<usize> {
        ...
        let decode_metadata = self
            .engine
            .internal_decode(b64_to_decode, buf, self.engine.internal_decoded_len_estimate(b64_len_to_decode))
            .map_err(|dse| match dse {
                DecodeSliceError::DecodeError(de) => {
                    match de {
                        ...
                        DecodeError::InvalidLength(len) => DecodeError::InvalidLength(self.input_consumed_len + len),
    ...
    fn read(&mut self, buf: &mut [u8]) -> io::Result<usize> {
        ...
        if self.decoded_len > 0 {
            ...
            if buf.len() < DECODED_CHUNK_SIZE {
                ...
                let actual_decode_len = cmp::min(b64_bytes_that_can_decode_into_buf, b64_bytes_available_to_decode);
