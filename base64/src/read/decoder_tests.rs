    ...
    for _ in 0..1_000 {
        ...
        for &s1 in alphabet.symbols.iter() {
            ...
            *b64_bytes.last_mut().unwrap() = s1;
            ...
            let stream_res = decoder
                .read_to_end(&mut decoded)
                .map(|_| ())
                .map_err(|e| e.into_inner().and_then(|e| e.downcast::<DecodeError>().ok()));
...
fn reports_invalid_byte_correctly() {
    ...
    for _ in 0..10_000 {
        ...
        let read_decode_err = decoder
            ...
            .map_err(|e| {
                let kind = e.kind();
                let inner = e.into_inner().and_then(|e| e.downcast::<DecodeError>().ok());
                inner.map(|i| (*i, kind))
            })
            .err()
            .and_then(|o| o);
        ...
        assert_eq!(bulk_decode_err.map(|e| (e, io::ErrorKind::InvalidData)), read_decode_err);
    }
}
...
fn internal_padding_error_with_short_read_concatenated_texts_invalid_byte_error() {
    ...
    for _ in 0..10_000 {
        ...
        let read_decode_err = decoder
            ...
            .map_err(|e| *e.into_inner().and_then(|e| e.downcast::<DecodeError>().ok()).unwrap())
            .unwrap_err();
