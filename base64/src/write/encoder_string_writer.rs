...
mod tests {
    ...
    use crate::{engine::Engine, tests::random_engine, write::encoder_string_writer::EncoderStringWriter};
    ...
    fn incremental_writes() {
        ...
        for _ in 0..size {
            ...
            while offset < size {
                ...
                let len = stream_encoder.write(&orig_data[offset..offset + nibble_size]).unwrap();
                offset += len;
            }
