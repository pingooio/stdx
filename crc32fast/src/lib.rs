...
impl Hasher {
    ...
    pub fn new_with_initial_len(init: u32, amount: u64) -> Self {
        Self::internal_new_specialized(init, amount).unwrap_or_else(|| Self::internal_new_baseline(init, amount))
    }
...
mod test {
    ...
    fn combine() {
        ...
        assert_eq!(hash_a.finalize(), hash_c.finalize());
    }
    ...
    fn combine_from_len() {
        ...
        assert_eq!(reconstructed.finalize(), ab);
    }
}
