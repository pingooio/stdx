...
pub use self::{
    days_of_month::DaysOfMonth, days_of_week::DaysOfWeek, hours::Hours, minutes::Minutes, months::Months,
    ...
    Self: Sized,
{
    ...
    fn ordinals_from_specifier(specifier: &Specifier) -> Result<OrdinalSet, Error> {
        ...
        match *specifier {
            ...
            Range(start, end) => match (Self::validate_ordinal(start), Self::validate_ordinal(end)) {
                (Ok(start), Ok(end)) if start <= end => Ok((start..end + 1).collect()),
                _ => {
                    Err(ErrorKind::Expression(format!("Invalid range for {}: {}-{}", Self::name(), start, end)).into())
                }
            },
