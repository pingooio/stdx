...
fn embedded(
  ...
  for embed_utils::FileEntry { rel_path, full_canonical_path } in embed_utils::get_files(absolute_folder_path.clone(), matcher) {
    match_values.insert(
      rel_path.clone(),
...
fn embed_file(
  ...
  let hash = file.metadata.sha3_256_hash();
  ...
  Ok(quote! {
       #closure_args {
        ...
        #crate_path::EmbeddedFile {
            ...
            metadata: #crate_path::Metadata::__embed_new([#(#hash),*], #last_modified, #created #mimetype_tokens)
        }
      }
  })
}
...
fn impl_embed(ast: &syn::DeriveInput) -> syn::Result<TokenStream2> {
  match ast.data {
  ...
  let crate_path: syn::Path = find_attribute_values(ast, "crate_path")
    ...
    .unwrap_or_else(|| syn::parse_str("embed").unwrap());
