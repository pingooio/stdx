...
use self::{Fail::*, HasArg::*, Name::*, Occur::*, Optval::*};
...
impl Options {
    ...
    pub fn short_usage(&self, program_name: &str) -> String {
        ...
        line.push_str(&self.grps.iter().map(format_option).collect::<Vec<String>>().join(" "));
        line
    }
...
impl Matches {
    ...
    pub fn opts_present(&self, names: &[String]) -> bool {
        names.iter().any(|nm| match find_opt(&self.opts, &Name::from_str(&nm)) {
    ...
    where
        C::Item: AsRef<str>,
    {
        names.into_iter().any(|nm| !self.opt_vals(nm.as_ref()).is_empty())
    }
