...
fn test_split_within() {
    ...
    t(
        "\nMary had a little lamb\nLittle lamb\n",
        ::std::usize::MAX,
        &["Mary had a little lamb".to_string(), "Little lamb".to_string()],
    );
}
...
fn test_reqopt_missing() {
    ...
    match Options::new().reqopt("t", "test", "testing", "TEST").parse(&args) {
        Err(OptionMissing(_)) => {}
        _ => panic!(),
    }
}
...
fn test_optopt_missing() {
    let args = vec!["blah".to_string()];
    match Options::new().optopt("t", "test", "testing", "TEST").parse(&args) {
        Ok(ref m) => {
            assert!(!m.opt_present("test"));
            assert!(!m.opt_present("t"));
        }
...
fn test_optflagmulti_short1() {
    ...
    match Options::new().optflagmulti("v", "verbose", "verbosity").parse(&args) {
        Ok(ref m) => {
...
fn test_optmulti_missing() {
    let args = vec!["blah".to_string()];
    match Options::new().optmulti("t", "test", "testing", "TEST").parse(&args) {
        Ok(ref m) => {
            assert!(!m.opt_present("test"));
            assert!(!m.opt_present("t"));
        }
...
fn test_mixed_stop_hyphen() {
    let args = vec!["-a".to_string(), "-".to_string(), "-c".to_string(), "d".to_string()];
    match Options::new()
        .parsing_style(ParsingStyle::StopAtFirstFree)
        .optflag("a", "", "")
        .optopt("c", "", "", "")
...
fn test_multi() {
    ...
    assert_eq!(matches_single.opts_str_first(&["e", "encrypt"]).unwrap(), "foo");
    assert_eq!(matches_single.opts_str_first(&["encrypt", "e"]).unwrap(), "foo");
    ...
    assert_eq!(matches_both.opts_str(&["encrypt".to_string()]).unwrap(), "bar");
    assert_eq!(
        matches_both
            .opts_str(&["e".to_string(), "encrypt".to_string()])
            .unwrap(),
    ...
    assert_eq!(matches_both.opts_str_first(&["e", "encrypt"]).unwrap(), "foo");
    assert_eq!(matches_both.opts_str_first(&["encrypt", "e"]).unwrap(), "bar");
...
fn test_usage_description_wrapping() {
    ...
    let mut opts = Options::new();
    opts.optflag("k", "kiwi", "This is a long description which won't be wrapped..+.."); // 54
    opts.optflag("a", "apple", "This is a long description which _will_ be wrapped..+..");
    opts.optflag(
        "b",
        "banana",
...
fn test_usage_description_multibyte_handling() {
    let mut opts = Options::new();
    opts.optflag("k", "k\u{2013}w\u{2013}", "The word kiwi is normally spelled with two i's");
    opts.optflag(
        "a",
        "apple",
...
fn test_args_with_equals() {
    ...
    let args = vec!["--one".to_string(), "A=B".to_string(), "--two=C=D".to_string()];
...
fn test_long_only_mode() {
    ...
    let args = vec!["-a", "A", "-b=B", "--c=C", "-durian", "D", "--e", "E", "-fruit=any"];
    let matches = &match opts.parse(&args) {
        Ok(m) => m,
        Err(e) => panic!("{}", e),
    };
...
fn test_opt_get() {
    ...
    let args: Vec<String> = ["-i", "true", "-p", "1.1"].iter().map(|x| x.to_string()).collect();
    let matches = &match opts.parse(&args) {
        Ok(m) => m,
        Err(e) => panic!("{}", e),
    };
