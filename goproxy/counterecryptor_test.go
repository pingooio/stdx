...
func TestCounterEncDifferentConsecutive(t *testing.T) {
	k, err := rsa.GenerateKey(&RandSeedReader{*rand.New(rand.NewPCG(0xFF43109, 0))}, 128)
...
func TestCounterEncIdenticalStreams(t *testing.T) {
	randSource := rand.NewPCG(0xFF43109, 0)

	k, err := rsa.GenerateKey(&RandSeedReader{*rand.New(randSource)}, 128)
	fatalOnErr(err, "rsa.GenerateKey", t)
