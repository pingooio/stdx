...
/// ```
///
/// *Note*: We need to keep input `AsRef<[u8]>` for backward compatibility, but it will be changed to
/// &[u8] in the future. (aws-sdk)
#[must_use]
#[cfg(feature = "alloc")]
...
pub fn encode_with_alphabet(data: &[u8], alphabet: Alphabet) -> String {
    match alphabet {
        Alphabet::Lower => data.encode_hex(),
        Alphabet::Upper => data.encode_hex_upper(),
    }
}
...
mod test {
    ...
    fn test_encode_to_slice() {
        ...
        assert_eq!(encode_to_slice(b"kiwis", &mut output_3), Err(FromHexError::InvalidStringLength));
    }
