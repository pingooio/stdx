...
pub fn generate_message_id_header(mut output: impl std::io::Write, hostname: &str) -> std::io::Result<()> {
    output.write_all(b"<")?;
...
impl<'x> Header for MessageId<'x> {
    fn write_header(&self, mut output: impl std::io::Write, mut bytes_written: usize) -> std::io::Result<usize> {
