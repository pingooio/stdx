...
fn build_rev_map<W: Write>(out: &mut W) {
    ...
    write!(out, "static REV_MAPPINGS: &'static [(&'static str, TopLevelExts)] = &[").unwrap();
    ...
    for (top, subs) in dyn_map {
        ...
        for (sub, sub_exts) in subs {
            ...
            write!(sub_map, "(\"{}\", ({}, {})),", sub, sub_start, sub_end).unwrap();
        }
...
#[cfg(feature = "rev-mappings")]
fn get_rev_mappings() -> BTreeMap<&'static str, BTreeMap<&'static str, Vec<&'static str>>> {
