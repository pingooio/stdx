...
import (
	...
	"github.com/pingooio/stdx/mmdb"
)

// This example shows how to decode to a struct.
func ExampleReader_Lookup_struct() {
	db, err := mmdb.Open("test-data/test-data/GeoIP2-City-Test.mmdb")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
...
func ExampleReader_Networks() {
	db, err := mmdb.Open("test-data/test-data/GeoIP2-Connection-Type-Test.mmdb")
	if err != nil {
		log.Fatal(err)
	}
	defer db.Close()
	...
	networks := db.Networks(mmdb.SkipAliasedNetworks)
	for networks.Next() {
...
func ExampleReader_NetworksWithin() {
	...
	networks := db.NetworksWithin(network, mmdb.SkipAliasedNetworks)
	for networks.Next() {
		subnet, err := networks.Network(&record)
		if err != nil {
			log.Panic(err)
