...
func TestNonEmptyNilInterface(t *testing.T) {
	...
	assert.Equal(
		t,
		"mmdb: cannot unmarshal map into type mmdb.TestInterface",
		err.Error(),
	)
}

type CityTraits struct {
	AutonomousSystemNumber uint `json:"autonomous_system_number,omitempty" mmdb:"autonomous_system_number"`
}
