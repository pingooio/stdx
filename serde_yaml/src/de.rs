...
use serde::de::{
    self, value::StrDeserializer, Deserialize, DeserializeOwned, DeserializeSeed, Expected, IgnoredAny, Unexpected,
    Visitor,
};
...
impl<'de, 'document> DeserializerFromEvents<'de, 'document> {
    ...
    fn jump<'anchor>(&'anchor mut self, pos: &'anchor mut usize) -> Result<DeserializerFromEvents<'de, 'anchor>> {
        *self.jumpcount += 1;
...
impl<'de, 'document, 'variant> de::VariantAccess<'de> for UnitVariantAccess<'de, 'document, 'variant> {
    type Error = Error;
    ...
    V: Visitor<'de>,
{
    let v = match str::from_utf8(&scalar.value) {
        Ok(v) => v,
        Err(_) => return Err(de::Error::invalid_type(Unexpected::Bytes(&scalar.value), &visitor)),
    };
...
fn parse_borrowed_str<'de>(utf8_value: &str, repr: Option<&'de [u8]>, style: ScalarStyle) -> Option<&'de str> {
...
fn parse_unsigned_int<T>(scalar: &str, from_str_radix: fn(&str, radix: u32) -> Result<T, ParseIntError>) -> Option<T> {
...
fn parse_signed_int<T>(scalar: &str, from_str_radix: fn(&str, radix: u32) -> Result<T, ParseIntError>) -> Option<T> {
...
fn parse_negative_int<T>(scalar: &str, from_str_radix: fn(&str, radix: u32) -> Result<T, ParseIntError>) -> Option<T> {
    if let Some(rest) = scalar.strip_prefix("-0x") {
        let negative = format!("-{}", rest);
        if let Ok(int) = from_str_radix(&negative, 16) {
            return Some(int);
                ...
                if scalar.style != ScalarStyle::Plain {
                    ...
                    if tag == Tag::NULL {
                        if let Some(()) = parse_null(&scalar.value) {
                            ...
                            return Err(de::Error::invalid_value(Unexpected::Bytes(&scalar.value), &"null"));
                        }
                    } else {
                        true
                    }
        ...
        match next {
            Event::Scalar(scalar) => {
                ...
                if is_null {
                    ...
                    Err(de::Error::invalid_value(Unexpected::Bytes(&scalar.value), &"null"))
                }
            }
            ...
            if let Some(current_enum) = self.current_enum {
                if let Event::Scalar(scalar) = next {
                    if !scalar.value.is_empty() {
                        break visitor.visit_enum(UnitVariantAccess {
                            de: self,
                        });
                    }
                }
