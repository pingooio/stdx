...
impl ser::Serializer for Serializer {
    ...
    fn serialize_newtype_variant<T>(self, _name: &str, _variant_index: u32, variant: &str, value: &T) -> Result<Value>
    where
        T: ?Sized + ser::Serialize,
    {
        if variant.is_empty() {
        ...
        V: ?Sized + ser::Serialize,
    {
        ...
        impl ser::Serializer for CheckForTag {
            ...
            where
                T: ?Sized + ser::Serialize,
            {
                Serializer.serialize_newtype_struct(name, value).map(MaybeTag::NotTag)
            }
