...
pub mod singleton_map {
    ...
    use serde::{
        ...
        ser::{self, Serialize, SerializeMap, SerializeStructVariant, SerializeTupleVariant, Serializer},
    };
        ...
        M: SerializeMap,
    {
        ...
        where
            T: ?Sized + Serialize,
        {
            let value = field.serialize(crate::value::Serializer).map_err(ser::Error::custom)?;
        ...
        V: Visitor<'de>,
    {
        ...
        where
            E: de::Error,
        {
            self.delegate.visit_enum(de::value::BorrowedStrDeserializer::new(v))
        }
        ...
        D: MapAccess<'de>,
    {
            ...
            V: DeserializeSeed<'de>,
        {
            match self.delegate.next_key_seed(seed)? {
                ...
                None => Err(de::Error::invalid_value(Unexpected::Map, &"map with a single key")),
            }
        }
    }
        ...
        D: MapAccess<'de>,
    {
        ...
        fn struct_variant<V>(mut self, fields: &'static [&'static str], visitor: V) -> Result<V::Value, Self::Error>
        where
            V: Visitor<'de>,
        {
...
pub mod singleton_map_recursive {
    ...
    use serde::{
        de::{
            self, Deserialize, DeserializeSeed, Deserializer, EnumAccess, IgnoredAny, MapAccess, SeqAccess, Unexpected,
            VariantAccess, Visitor,
        },
        ser::{
