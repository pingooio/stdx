...
type Children = BTreeMap<Vec<u8>, Node>;
...
impl List {
    ...
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, Error> {
        from_utf8(bytes).map_err(|_| Error::ListNotUtf8Encoded)?.parse()
    }
            ...
            match line {
                ...
                line => match typ {
                    Some(typ) => {
                        ...
                        {
                            let ascii = idna::domain_to_ascii(rule).map_err(|_| Error::InvalidRule(rule.to_owned()))?;
