...
pub(crate) unsafe fn yaml_string_extend(
    ...
    memset(
        new_start.wrapping_offset((*end).c_offset_from(*start) as libc::c_long as isize) as *mut libc::c_void,
        0,
    ...
    *end = new_start.wrapping_offset((((*end).c_offset_from(*start) as libc::c_long).force_mul(2_i64)) as isize);
    *start = new_start;
}
...
pub(crate) unsafe fn yaml_string_join(
    ...
    while (*a_end).c_offset_from(*a_pointer) as libc::c_long <= (*b_pointer).c_offset_from(*b_start) as libc::c_long {
    ...
    *a_pointer = (*a_pointer).wrapping_offset((*b_pointer).c_offset_from(*b_start) as libc::c_long as isize);
}
...
pub(crate) unsafe fn yaml_stack_extend(
    ...
    let new_start: *mut libc::c_void = yaml_realloc(
        *start,
        (((*end as *mut libc::c_char).c_offset_from(*start as *mut libc::c_char) as libc::c_long).force_mul(2_i64))
            as size_t,
    );
    *top = (new_start as *mut libc::c_char).wrapping_offset(
        (*top as *mut libc::c_char).c_offset_from(*start as *mut libc::c_char) as libc::c_long as isize,
    ) as *mut libc::c_void;
...
pub(crate) unsafe fn yaml_queue_extend(
    ...
    if *start == *head && *tail == *end {
        ...
        *head = (new_start as *mut libc::c_char).wrapping_offset(
            (*head as *mut libc::c_char).c_offset_from(*start as *mut libc::c_char) as libc::c_long as isize,
        ) as *mut libc::c_void;
        *tail = (new_start as *mut libc::c_char).wrapping_offset(
            (*tail as *mut libc::c_char).c_offset_from(*start as *mut libc::c_char) as libc::c_long as isize,
        ) as *mut libc::c_void;
        *end = (new_start as *mut libc::c_char).wrapping_offset(
            (((*end as *mut libc::c_char).c_offset_from(*start as *mut libc::c_char) as libc::c_long).force_mul(2_i64))
                as isize,
    ...
    if *tail == *end {
        if *head != *tail {
            memmove(
                *start,
                *head,
                (*tail as *mut libc::c_char).c_offset_from(*head as *mut libc::c_char) as libc::c_ulong,
            );
        }
        *tail = (*start as *mut libc::c_char).wrapping_offset(
            (*tail as *mut libc::c_char).c_offset_from(*head as *mut libc::c_char) as libc::c_long as isize,
        ) as *mut libc::c_void;
        *head = *start;
    }
}
...
pub unsafe fn yaml_parser_initialize(parser: *mut yaml_parser_t) -> Success {
    ...
    memset(parser as *mut libc::c_void, 0, size_of::<yaml_parser_t>() as libc::c_ulong);
...
unsafe fn yaml_string_read_handler(
    ...
    if size > (*parser).input.string.end.c_offset_from((*parser).input.string.current) as size_t {
        size = (*parser).input.string.end.c_offset_from((*parser).input.string.current) as size_t;
    }
    memcpy(
...
pub unsafe fn yaml_parser_set_input_string(parser: *mut yaml_parser_t, input: *const libc::c_uchar, size: size_t) {
    __assert!(!parser.is_null());
...
pub unsafe fn yaml_parser_set_input(parser: *mut yaml_parser_t, handler: yaml_read_handler_t, data: *mut libc::c_void) {
    __assert!(!parser.is_null());
    __assert!(((*parser).read_handler).is_none());
...
pub unsafe fn yaml_emitter_initialize(emitter: *mut yaml_emitter_t) -> Success {
    ...
    memset(emitter as *mut libc::c_void, 0, size_of::<yaml_emitter_t>() as libc::c_ulong);
...
unsafe fn yaml_string_write_handler(data: *mut libc::c_void, buffer: *mut libc::c_uchar, size: size_t) -> libc::c_int {
        ...
        < size
    {
        memcpy(
            (*emitter)
                ...
                .string
                .buffer
                .wrapping_offset(*(*emitter).output.string.size_written as isize) as *mut libc::c_void,
...
pub unsafe fn yaml_emitter_set_output_string(
    ...
    *fresh154 =
        Some(yaml_string_write_handler as unsafe fn(*mut libc::c_void, *mut libc::c_uchar, size_t) -> libc::c_int);
...
pub unsafe fn yaml_stream_start_event_initialize(event: *mut yaml_event_t, encoding: yaml_encoding_t) -> Success {
...
pub unsafe fn yaml_document_start_event_initialize(
    ...
    let mut version_directive_copy: *mut yaml_version_directive_t = ptr::null_mut::<yaml_version_directive_t>();
    ...
    __assert!(!event.is_null());
    __assert!(
        !tag_directives_start.is_null() && !tag_directives_end.is_null() || tag_directives_start == tag_directives_end
    );
    if !version_directive.is_null() {
        version_directive_copy =
            yaml_malloc(size_of::<yaml_version_directive_t>() as libc::c_ulong) as *mut yaml_version_directive_t;
    ...
    if tag_directives_start != tag_directives_end {
        ...
        loop {
            ...
            if yaml_check_utf8((*tag_directive).handle, strlen((*tag_directive).handle as *mut libc::c_char)).fail {
                current_block = 14964981520188694172;
                break;
            }
            if yaml_check_utf8((*tag_directive).prefix, strlen((*tag_directive).prefix as *mut libc::c_char)).fail {
                current_block = 14964981520188694172;
                break;
            }
...
/// The `implicit` argument is considered as a stylistic parameter and may be
/// ignored by the emitter.
pub unsafe fn yaml_document_end_event_initialize(event: *mut yaml_event_t, implicit: bool) -> Success {
    let mark = yaml_mark_t {
        index: 0_u64,
        line: 0_u64,
        column: 0_u64,
...
pub unsafe fn yaml_alias_event_initialize(event: *mut yaml_event_t, anchor: *const yaml_char_t) -> Success {
    let mark = yaml_mark_t {
        index: 0_u64,
        line: 0_u64,
        column: 0_u64,
...
pub unsafe fn yaml_document_initialize(
    ...
    if current_block != 8142820162064489797 {
        memset(document as *mut libc::c_void, 0, size_of::<yaml_document_t>() as libc::c_ulong);
...
pub unsafe fn yaml_document_get_node(document: *mut yaml_document_t, index: libc::c_int) -> *mut yaml_node_t {
    __assert!(!document.is_null());
    ...
    if yaml_check_utf8(tag, strlen(tag as *mut libc::c_char)).ok {
        ...
        if !tag_copy.is_null() {
            ...
            if yaml_check_utf8(value, length as size_t).ok {
                ...
                *value_copy.wrapping_offset(length as isize) = b'\0';
                memset(node as *mut libc::c_void, 0, size_of::<yaml_node_t>() as libc::c_ulong);
...
pub unsafe fn yaml_document_append_sequence_item(
    ...
    __assert!(!document.is_null());
    __assert!(sequence > 0 && ((*document).nodes.start).wrapping_offset(sequence as isize) <= (*document).nodes.top);
    __assert!((*((*document).nodes.start).wrapping_offset((sequence - 1) as isize)).type_ == YAML_SEQUENCE_NODE);
    __assert!(item > 0 && ((*document).nodes.start).wrapping_offset(item as isize) <= (*document).nodes.top);
    PUSH!(
...
pub unsafe fn yaml_document_append_mapping_pair(
    ...
    __assert!(!document.is_null());
    __assert!(mapping > 0 && ((*document).nodes.start).wrapping_offset(mapping as isize) <= (*document).nodes.top);
