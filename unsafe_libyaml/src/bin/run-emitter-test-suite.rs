...
pub(crate) unsafe fn unsafe_main(stdin: &mut dyn Read, mut stdout: &mut dyn Write) -> Result<(), Box<dyn Error>> {
    ...
    let result = loop {
        ...
        if yaml_emitter_emit(emitter, event).fail {
            break Err(match (*emitter).error {
                ...
                YAML_WRITER_ERROR => format!("Writer error: {}", CStr::from_ptr((*emitter).problem)).into(),
                YAML_EMITTER_ERROR => format!("Emitter error: {}", CStr::from_ptr((*emitter).problem)).into(),
                // Couldn't happen.
    ...
    fn get_line(&mut self, input: &mut dyn Read) -> Option<&mut [u8]> {
        loop {
            ...
            if remainder.is_empty() {
                if self.offset == 0 {
                    let _ = writeln!(io::stderr(), "Line too long: '{}'", String::from_utf8_lossy(&self.buf),);
                    process::abort();
                }
