...
pub unsafe fn yaml_emitter_dump(emitter: *mut yaml_emitter_t, document: *mut yaml_document_t) -> Success {
            ...
            if STACK_EMPTY!((*document).nodes) {
                ...
                *fresh1 = yaml_malloc(
                    (size_of::<yaml_anchors_t>() as libc::c_ulong)
                        .force_mul((*document).nodes.top.c_offset_from((*document).nodes.start) as libc::c_ulong),
...
unsafe fn yaml_emitter_delete_document_and_anchors(emitter: *mut yaml_emitter_t) {
    ...
    index = 0;
    while (*(*emitter).document).nodes.start.wrapping_offset(index as isize) < (*(*emitter).document).nodes.top {
        let mut node: yaml_node_t = *(*(*emitter).document).nodes.start.wrapping_offset(index as isize);
...
unsafe fn yaml_emitter_anchor_node(emitter: *mut yaml_emitter_t, index: libc::c_int) {
    ...
    let fresh8 = addr_of_mut!((*((*emitter).anchors).wrapping_offset((index - 1) as isize)).references);
    *fresh8 += 1;
...
unsafe fn yaml_emitter_generate_anchor(_emitter: *mut yaml_emitter_t, anchor_id: libc::c_int) -> *mut yaml_char_t {
...
unsafe fn yaml_emitter_dump_alias(emitter: *mut yaml_emitter_t, anchor: *mut yaml_char_t) -> Success {
    let mut event = MaybeUninit::<yaml_event_t>::uninit();
    let event = event.as_mut_ptr();
    let mark = yaml_mark_t {
        index: 0_u64,
