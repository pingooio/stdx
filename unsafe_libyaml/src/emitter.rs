...
use crate::{
    api::{yaml_free, yaml_queue_extend, yaml_stack_extend, yaml_strdup},
    externs::{strcmp, strlen, strncmp},
    libc,
    ops::{ForceAdd as _, ForceMul as _},
    success::{Success, FAIL, OK},
    yaml::{size_t, yaml_char_t, yaml_string_t},
    yaml_emitter_flush, yaml_emitter_t, yaml_event_delete, yaml_event_t, yaml_scalar_style_t, yaml_tag_directive_t,
    yaml_version_directive_t, PointerExt, YAML_ALIAS_EVENT, YAML_ANY_BREAK, YAML_ANY_ENCODING, YAML_ANY_SCALAR_STYLE,
    ...
    YAML_EMIT_BLOCK_MAPPING_KEY_STATE, YAML_EMIT_BLOCK_MAPPING_SIMPLE_VALUE_STATE, YAML_EMIT_BLOCK_MAPPING_VALUE_STATE,
    YAML_EMIT_BLOCK_SEQUENCE_FIRST_ITEM_STATE, YAML_EMIT_BLOCK_SEQUENCE_ITEM_STATE, YAML_EMIT_DOCUMENT_CONTENT_STATE,
    YAML_EMIT_DOCUMENT_END_STATE, YAML_EMIT_DOCUMENT_START_STATE, YAML_EMIT_END_STATE,
    YAML_EMIT_FIRST_DOCUMENT_START_STATE, YAML_EMIT_FLOW_MAPPING_FIRST_KEY_STATE, YAML_EMIT_FLOW_MAPPING_KEY_STATE,
    YAML_EMIT_FLOW_MAPPING_SIMPLE_VALUE_STATE, YAML_EMIT_FLOW_MAPPING_VALUE_STATE,
    YAML_EMIT_FLOW_SEQUENCE_FIRST_ITEM_STATE, YAML_EMIT_FLOW_SEQUENCE_ITEM_STATE, YAML_EMIT_STREAM_START_STATE,
    YAML_FLOW_MAPPING_STYLE, YAML_FLOW_SEQUENCE_STYLE, YAML_FOLDED_SCALAR_STYLE, YAML_LITERAL_SCALAR_STYLE,
    YAML_LN_BREAK, YAML_MAPPING_END_EVENT, YAML_MAPPING_START_EVENT, YAML_PLAIN_SCALAR_STYLE, YAML_SCALAR_EVENT,
    YAML_SEQUENCE_END_EVENT, YAML_SEQUENCE_START_EVENT, YAML_SINGLE_QUOTED_SCALAR_STYLE, YAML_STREAM_END_EVENT,
...
unsafe fn yaml_emitter_set_emitter_error(emitter: *mut yaml_emitter_t, problem: *const libc::c_char) -> Success {
...
unsafe fn yaml_emitter_need_more_events(emitter: *mut yaml_emitter_t) -> Success {
    ...
    if (*emitter).events.tail.c_offset_from((*emitter).events.head) as libc::c_long > accumulate as libc::c_long {
        return FAIL;
    }
    ...
    while event != (*emitter).events.tail {
        match (*event).type_ {
            ...
            YAML_STREAM_END_EVENT | YAML_DOCUMENT_END_EVENT | YAML_SEQUENCE_END_EVENT | YAML_MAPPING_END_EVENT => {
                level -= 1;
            }
            _ => {}
        }
...
unsafe fn yaml_emitter_append_tag_directive(
    ...
    while tag_directive != (*emitter).tag_directives.top {
        if strcmp(value.handle as *mut libc::c_char, (*tag_directive).handle as *mut libc::c_char) == 0 {
            if allow_duplicates {
                return OK;
            }
...
unsafe fn yaml_emitter_state_machine(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    match (*emitter).state {
        YAML_EMIT_STREAM_START_STATE => yaml_emitter_emit_stream_start(emitter, event),
        YAML_EMIT_FIRST_DOCUMENT_START_STATE => yaml_emitter_emit_document_start(emitter, event, true),
        ...
        YAML_EMIT_FLOW_SEQUENCE_FIRST_ITEM_STATE => yaml_emitter_emit_flow_sequence_item(emitter, event, true),
        YAML_EMIT_FLOW_SEQUENCE_ITEM_STATE => yaml_emitter_emit_flow_sequence_item(emitter, event, false),
        YAML_EMIT_FLOW_MAPPING_FIRST_KEY_STATE => yaml_emitter_emit_flow_mapping_key(emitter, event, true),
        YAML_EMIT_FLOW_MAPPING_KEY_STATE => yaml_emitter_emit_flow_mapping_key(emitter, event, false),
        YAML_EMIT_FLOW_MAPPING_SIMPLE_VALUE_STATE => yaml_emitter_emit_flow_mapping_value(emitter, event, true),
        YAML_EMIT_FLOW_MAPPING_VALUE_STATE => yaml_emitter_emit_flow_mapping_value(emitter, event, false),
        YAML_EMIT_BLOCK_SEQUENCE_FIRST_ITEM_STATE => yaml_emitter_emit_block_sequence_item(emitter, event, true),
        YAML_EMIT_BLOCK_SEQUENCE_ITEM_STATE => yaml_emitter_emit_block_sequence_item(emitter, event, false),
        YAML_EMIT_BLOCK_MAPPING_FIRST_KEY_STATE => yaml_emitter_emit_block_mapping_key(emitter, event, true),
        YAML_EMIT_BLOCK_MAPPING_KEY_STATE => yaml_emitter_emit_block_mapping_key(emitter, event, false),
        YAML_EMIT_BLOCK_MAPPING_SIMPLE_VALUE_STATE => yaml_emitter_emit_block_mapping_value(emitter, event, true),
        YAML_EMIT_BLOCK_MAPPING_VALUE_STATE => yaml_emitter_emit_block_mapping_value(emitter, event, false),
...
unsafe fn yaml_emitter_emit_stream_start(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    ...
    yaml_emitter_set_emitter_error(emitter, b"expected STREAM-START\0" as *const u8 as *const libc::c_char)
}
...
unsafe fn yaml_emitter_emit_document_start(
    ...
    if (*event).type_ == YAML_DOCUMENT_START_EVENT {
        ...
        if !(*event).data.document_start.version_directive.is_null() {
            if yaml_emitter_analyze_version_directive(emitter, *(*event).data.document_start.version_directive).fail {
                return FAIL;
            }
        }
        ...
        if (!(*event).data.document_start.version_directive.is_null()
            ...
            && (*emitter).open_ended != 0
        {
            if yaml_emitter_write_indicator(emitter, b"...\0" as *const u8 as *const libc::c_char, true, false, false)
                .fail
            {
                return FAIL;
            }
        ...
        if !(*event).data.document_start.version_directive.is_null() {
            implicit = false;
            if yaml_emitter_write_indicator(emitter, b"%YAML\0" as *const u8 as *const libc::c_char, true, false, false)
                .fail
            {
                return FAIL;
            }
...
unsafe fn yaml_emitter_emit_document_content(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    PUSH!((*emitter).states, YAML_EMIT_DOCUMENT_END_STATE);
    yaml_emitter_emit_node(emitter, event, true, false, false, false)
}

unsafe fn yaml_emitter_emit_document_end(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    ...
    yaml_emitter_set_emitter_error(emitter, b"expected DOCUMENT-END\0" as *const u8 as *const libc::c_char)
}
...
unsafe fn yaml_emitter_emit_flow_sequence_item(
    ...
    if first {
        if yaml_emitter_write_indicator(emitter, b"[\0" as *const u8 as *const libc::c_char, true, true, false).fail {
            return FAIL;
        }
    ...
    if (*event).type_ == YAML_SEQUENCE_END_EVENT {
        ...
        *fresh13 -= 1;
        ...
        if (*emitter).canonical && !first {
            if yaml_emitter_write_indicator(emitter, b",\0" as *const u8 as *const libc::c_char, false, false, false)
                .fail
            {
                return FAIL;
            }
        ...
        if yaml_emitter_write_indicator(emitter, b"]\0" as *const u8 as *const libc::c_char, false, false, false).fail {
            return FAIL;
        }
...
unsafe fn yaml_emitter_emit_flow_mapping_key(
    ...
    if !(*emitter).canonical && yaml_emitter_check_simple_key(emitter) {
        ...
        if yaml_emitter_write_indicator(emitter, b"?\0" as *const u8 as *const libc::c_char, true, false, false).fail {
            return FAIL;
        }
...
unsafe fn yaml_emitter_emit_block_sequence_item(
    ...
    if first {
        yaml_emitter_increase_indent(emitter, false, (*emitter).mapping_context && !(*emitter).indention);
    }
    if (*event).type_ == YAML_SEQUENCE_END_EVENT {
        (*emitter).indent = POP!((*emitter).indents);
        (*emitter).state = POP!((*emitter).states);
    ...
    if yaml_emitter_write_indicator(emitter, b"-\0" as *const u8 as *const libc::c_char, true, false, true).fail {
        return FAIL;
    }
...
unsafe fn yaml_emitter_emit_node(
    ...
    match (*event).type_ {
        ...
        _ => yaml_emitter_set_emitter_error(
            emitter,
            b"expected SCALAR, SEQUENCE-START, MAPPING-START, or ALIAS\0" as *const u8 as *const libc::c_char,
        ),
    }
}

unsafe fn yaml_emitter_emit_alias(emitter: *mut yaml_emitter_t, _event: *mut yaml_event_t) -> Success {
...
unsafe fn yaml_emitter_emit_scalar(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
...
unsafe fn yaml_emitter_emit_sequence_start(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    if yaml_emitter_process_anchor(emitter).fail {
        return FAIL;
    }
    if yaml_emitter_process_tag(emitter).fail {
...
unsafe fn yaml_emitter_emit_mapping_start(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    if yaml_emitter_process_anchor(emitter).fail {
        return FAIL;
    }
    if yaml_emitter_process_tag(emitter).fail {
...
unsafe fn yaml_emitter_check_simple_key(emitter: *mut yaml_emitter_t) -> bool {
    ...
    match (*event).type_ {
        YAML_ALIAS_EVENT => {
            length = (length as libc::c_ulong).force_add((*emitter).anchor_data.anchor_length) as size_t;
        }
        YAML_SCALAR_EVENT => {
...
unsafe fn yaml_emitter_select_scalar_style(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    ...
    if style == YAML_PLAIN_SCALAR_STYLE {
        ...
        if (*emitter).scalar_data.length == 0 && ((*emitter).flow_level != 0 || (*emitter).simple_key_context) {
            style = YAML_SINGLE_QUOTED_SCALAR_STYLE;
        }
...
unsafe fn yaml_emitter_process_tag(emitter: *mut yaml_emitter_t) -> Success {
    ...
    if !(*emitter).tag_data.handle.is_null() {
        if yaml_emitter_write_tag_handle(emitter, (*emitter).tag_data.handle, (*emitter).tag_data.handle_length).fail {
            return FAIL;
        }
        ...
        if yaml_emitter_write_tag_content(emitter, (*emitter).tag_data.suffix, (*emitter).tag_data.suffix_length, false)
            .fail
        {
            return FAIL;
        }
...
unsafe fn yaml_emitter_analyze_tag_directive(
    ...
    while handle.pointer < handle.end.wrapping_offset(-1_isize) {
        if !IS_ALPHA!(handle) {
            return yaml_emitter_set_emitter_error(
                emitter,
                b"tag handle must contain alphanumerical characters only\0" as *const u8 as *const libc::c_char,
            );
        }
        MOVE!(handle);
    }
...
unsafe fn yaml_emitter_analyze_anchor(emitter: *mut yaml_emitter_t, anchor: *mut yaml_char_t, alias: bool) -> Success {
    ...
    while string.pointer != string.end {
        if !IS_ALPHA!(string) {
            return yaml_emitter_set_emitter_error(
                emitter,
                if alias {
                    b"alias value must contain alphanumerical characters only\0" as *const u8 as *const libc::c_char
                } else {
                    b"anchor value must contain alphanumerical characters only\0" as *const u8 as *const libc::c_char
                },
            );
        }
        MOVE!(string);
    ...
    while tag_directive != (*emitter).tag_directives.top {
            ...
            ) == 0
        {
            ...
            *fresh49 = string.start.wrapping_offset(prefix_length as isize);
            (*emitter).tag_data.suffix_length =
                (string.end.c_offset_from(string.start) as libc::c_ulong).wrapping_sub(prefix_length);
            return OK;
        }
        tag_directive = tag_directive.wrapping_offset(1);
    }
...
unsafe fn yaml_emitter_analyze_event(emitter: *mut yaml_emitter_t, event: *mut yaml_event_t) -> Success {
    ...
    *fresh55 = ptr::null_mut::<yaml_char_t>();
    ...
    match (*event).type_ {
        ...
        YAML_SCALAR_EVENT => {
            ...
            yaml_emitter_analyze_scalar(emitter, (*event).data.scalar.value, (*event).data.scalar.length)
        }
        YAML_SEQUENCE_START_EVENT => {
            if !(*event).data.sequence_start.anchor.is_null() {
                if yaml_emitter_analyze_anchor(emitter, (*event).data.sequence_start.anchor, false).fail {
                    return FAIL;
                }
            }
        ...
        YAML_MAPPING_START_EVENT => {
            if !(*event).data.mapping_start.anchor.is_null() {
                if yaml_emitter_analyze_anchor(emitter, (*event).data.mapping_start.anchor, false).fail {
                    return FAIL;
                }
            }
...
unsafe fn yaml_emitter_write_indent(emitter: *mut yaml_emitter_t) -> Success {
    let indent: libc::c_int = if (*emitter).indent >= 0 { (*emitter).indent } else { 0 };
...
unsafe fn yaml_emitter_write_anchor(emitter: *mut yaml_emitter_t, value: *mut yaml_char_t, length: size_t) -> Success {
    let mut string = STRING_ASSIGN!(value, length);
    while string.pointer != string.end {
        if WRITE!(emitter, string).fail {
            return FAIL;
            ...
            match value_0 {
                ...
                _ => {
                    ...
                    while k >= 0 {
                        ...
                        if PUT(emitter, (digit + if digit < 10 { b'0' } else { b'A' - 10 } as i32) as u8).fail {
                            return FAIL;
                        }
                        k -= 4;
                    }
...
unsafe fn yaml_emitter_write_block_scalar_hints(emitter: *mut yaml_emitter_t, mut string: yaml_string_t) -> Success {
