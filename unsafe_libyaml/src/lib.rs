...
mod externs {
    ...
    pub unsafe fn malloc(size: libc::c_ulong) -> *mut libc::c_void {
        ...
        let layout = Layout::from_size_align(size, MALLOC_ALIGN).ok().unwrap_or_else(die);
    ...
    pub unsafe fn realloc(ptr: *mut libc::c_void, new_size: libc::c_ulong) -> *mut libc::c_void {
        ...
        let new_layout = Layout::from_size_align(new_size, MALLOC_ALIGN).ok().unwrap_or_else(die);
    ...
    pub unsafe fn memcmp(lhs: *const libc::c_void, rhs: *const libc::c_void, count: libc::c_ulong) -> libc::c_int {
    ...
    pub unsafe fn memcpy(dest: *mut libc::c_void, src: *const libc::c_void, count: libc::c_ulong) -> *mut libc::c_void {
        ptr::copy_nonoverlapping(src.cast::<MaybeUninit<u8>>(), dest.cast::<MaybeUninit<u8>>(), count as usize);
        dest
    }
    ...
    pub unsafe fn memmove(
        ...
        ptr::copy(src.cast::<MaybeUninit<u8>>(), dest.cast::<MaybeUninit<u8>>(), count as usize);
        dest
    }

    pub unsafe fn memset(dest: *mut libc::c_void, ch: libc::c_int, count: libc::c_ulong) -> *mut libc::c_void {
    ...
    pub unsafe fn strncmp(lhs: *const libc::c_char, rhs: *const libc::c_char, mut count: libc::c_ulong) -> libc::c_int {
    ...
    pub(crate) unsafe fn __assert_fail(__assertion: &'static str, __file: &'static str, __line: u32) -> ! {
        struct Abort;
        impl Drop for Abort {
            fn drop(&mut self) {
                panic!();
...
#[doc(hidden)]
pub use crate::yaml::{
    yaml_break_t::*, yaml_emitter_state_t::*, yaml_encoding_t::*, yaml_error_type_t::*, yaml_event_type_t::*,
    yaml_mapping_style_t::*, yaml_node_type_t::*, yaml_parser_state_t::*, yaml_scalar_style_t::*,
    yaml_sequence_style_t::*, yaml_token_type_t::*,
};
pub use crate::{
    api::{
        yaml_alias_event_initialize, yaml_document_add_mapping, yaml_document_add_scalar, yaml_document_add_sequence,
        yaml_document_append_mapping_pair, yaml_document_append_sequence_item, yaml_document_delete,
        yaml_document_end_event_initialize, yaml_document_get_node, yaml_document_get_root_node,
        yaml_document_initialize, yaml_document_start_event_initialize, yaml_emitter_delete, yaml_emitter_initialize,
        yaml_emitter_set_break, yaml_emitter_set_canonical, yaml_emitter_set_encoding, yaml_emitter_set_indent,
        yaml_emitter_set_output, yaml_emitter_set_output_string, yaml_emitter_set_unicode, yaml_emitter_set_width,
        yaml_event_delete, yaml_mapping_end_event_initialize, yaml_mapping_start_event_initialize, yaml_parser_delete,
        yaml_parser_initialize, yaml_parser_set_encoding, yaml_parser_set_input, yaml_parser_set_input_string,
        yaml_scalar_event_initialize, yaml_sequence_end_event_initialize, yaml_sequence_start_event_initialize,
        yaml_stream_end_event_initialize, yaml_stream_start_event_initialize, yaml_token_delete,
    },
    dumper::{yaml_emitter_close, yaml_emitter_dump, yaml_emitter_open},
    emitter::yaml_emitter_emit,
    loader::yaml_parser_load,
    parser::yaml_parser_parse,
    scanner::yaml_parser_scan,
    writer::yaml_emitter_flush,
    yaml::{
        yaml_alias_data_t, yaml_break_t, yaml_document_t, yaml_emitter_state_t, yaml_emitter_t, yaml_encoding_t,
        yaml_error_type_t, yaml_event_t, yaml_event_type_t, yaml_mapping_style_t, yaml_mark_t, yaml_node_item_t,
        yaml_node_pair_t, yaml_node_t, yaml_node_type_t, yaml_parser_state_t, yaml_parser_t, yaml_read_handler_t,
        yaml_scalar_style_t, yaml_sequence_style_t, yaml_simple_key_t, yaml_stack_t, yaml_tag_directive_t,
        yaml_token_t, yaml_token_type_t, yaml_version_directive_t, yaml_write_handler_t,
    },
};
