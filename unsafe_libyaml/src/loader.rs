...
/// the parser.
pub unsafe fn yaml_parser_load(parser: *mut yaml_parser_t, document: *mut yaml_document_t) -> Success {
...
unsafe fn yaml_parser_load_document(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
...
unsafe fn yaml_parser_register_anchor(
    ...
    (*data).anchor = anchor;
    (*data).index = index;
    (*data).mark = (*(*(*parser).document).nodes.start.wrapping_offset((index - 1) as isize)).start_mark;
    alias_data = (*parser).aliases.start;
    while alias_data != (*parser).aliases.top {
        if strcmp((*alias_data).anchor as *mut libc::c_char, anchor as *mut libc::c_char) == 0 {
...
unsafe fn yaml_parser_load_node_add(parser: *mut yaml_parser_t, ctx: *mut loader_ctx, index: libc::c_int) -> Success {
    if STACK_EMPTY!(*ctx) {
        return OK;
    }
    let parent_index: libc::c_int = *(*ctx).top.wrapping_offset(-1_isize);
    let parent: *mut yaml_node_t =
        addr_of_mut!(*((*(*parser).document).nodes.start).wrapping_offset((parent_index - 1) as isize));
    ...
    match (*parent).type_ {
        ...
        YAML_MAPPING_NODE => {
            ...
            if !STACK_EMPTY!((*parent).data.mapping.pairs) {
                let p: *mut yaml_node_pair_t = (*parent).data.mapping.pairs.top.wrapping_offset(-1_isize);
...
unsafe fn yaml_parser_load_scalar(
    ...
    if STACK_LIMIT!(parser, (*(*parser).document).nodes).ok {
        if tag.is_null() || strcmp(tag as *mut libc::c_char, b"!\0" as *const u8 as *const libc::c_char) == 0 {
            yaml_free(tag as *mut libc::c_void);
            tag = yaml_strdup(b"tag:yaml.org,2002:str\0" as *const u8 as *const libc::c_char as *mut yaml_char_t);
            if tag.is_null() {
...
unsafe fn yaml_parser_load_sequence(
    ...
    if STACK_LIMIT!(parser, (*(*parser).document).nodes).ok {
        if tag.is_null() || strcmp(tag as *mut libc::c_char, b"!\0" as *const u8 as *const libc::c_char) == 0 {
            yaml_free(tag as *mut libc::c_void);
            tag = yaml_strdup(b"tag:yaml.org,2002:seq\0" as *const u8 as *const libc::c_char as *mut yaml_char_t);
            if tag.is_null() {
...
unsafe fn yaml_parser_load_sequence_end(
    ...
    __assert!((*((*(*parser).document).nodes.start).wrapping_offset((index - 1) as isize)).type_ == YAML_SEQUENCE_NODE);
    (*(*(*parser).document).nodes.start.wrapping_offset((index - 1) as isize)).end_mark = (*event).end_mark;
    let _ = POP!(*ctx);
    OK
}
...
unsafe fn yaml_parser_load_mapping(
    ...
    if STACK_LIMIT!(parser, (*(*parser).document).nodes).ok {
        if tag.is_null() || strcmp(tag as *mut libc::c_char, b"!\0" as *const u8 as *const libc::c_char) == 0 {
            yaml_free(tag as *mut libc::c_void);
            tag = yaml_strdup(b"tag:yaml.org,2002:map\0" as *const u8 as *const libc::c_char as *mut yaml_char_t);
            if tag.is_null() {
...
unsafe fn yaml_parser_load_mapping_end(
    ...
    __assert!(((*ctx).top).c_offset_from((*ctx).start) as libc::c_long > 0_i64);
    let index: libc::c_int = *(*ctx).top.wrapping_offset(-1_isize);
    __assert!((*((*(*parser).document).nodes.start).wrapping_offset((index - 1) as isize)).type_ == YAML_MAPPING_NODE);
    (*(*(*parser).document).nodes.start.wrapping_offset((index - 1) as isize)).end_mark = (*event).end_mark;
    let _ = POP!(*ctx);
    OK
}
