...
use crate::{
    ...
    yaml_event_t, yaml_mark_t, yaml_parser_t, yaml_tag_directive_t, yaml_token_t, yaml_version_directive_t,
    YAML_ALIAS_EVENT, YAML_ALIAS_TOKEN, YAML_ANCHOR_TOKEN, YAML_BLOCK_END_TOKEN, YAML_BLOCK_ENTRY_TOKEN,
    YAML_BLOCK_MAPPING_START_TOKEN, YAML_BLOCK_MAPPING_STYLE, YAML_BLOCK_SEQUENCE_START_TOKEN,
    YAML_BLOCK_SEQUENCE_STYLE, YAML_DOCUMENT_END_EVENT, YAML_DOCUMENT_END_TOKEN, YAML_DOCUMENT_START_EVENT,
    YAML_DOCUMENT_START_TOKEN, YAML_FLOW_ENTRY_TOKEN, YAML_FLOW_MAPPING_END_TOKEN, YAML_FLOW_MAPPING_START_TOKEN,
    YAML_FLOW_MAPPING_STYLE, YAML_FLOW_SEQUENCE_END_TOKEN, YAML_FLOW_SEQUENCE_START_TOKEN, YAML_FLOW_SEQUENCE_STYLE,
    YAML_KEY_TOKEN, YAML_MAPPING_END_EVENT, YAML_MAPPING_START_EVENT, YAML_NO_ERROR, YAML_PARSER_ERROR,
    YAML_PARSE_BLOCK_MAPPING_FIRST_KEY_STATE, YAML_PARSE_BLOCK_MAPPING_KEY_STATE, YAML_PARSE_BLOCK_MAPPING_VALUE_STATE,
    ...
    YAML_PARSE_DOCUMENT_CONTENT_STATE, YAML_PARSE_DOCUMENT_END_STATE, YAML_PARSE_DOCUMENT_START_STATE,
    YAML_PARSE_END_STATE, YAML_PARSE_FLOW_MAPPING_EMPTY_VALUE_STATE, YAML_PARSE_FLOW_MAPPING_FIRST_KEY_STATE,
    YAML_PARSE_FLOW_MAPPING_KEY_STATE, YAML_PARSE_FLOW_MAPPING_VALUE_STATE, YAML_PARSE_FLOW_NODE_STATE,
    ...
    YAML_PARSE_INDENTLESS_SEQUENCE_ENTRY_STATE, YAML_PARSE_STREAM_START_STATE, YAML_PLAIN_SCALAR_STYLE,
    YAML_SCALAR_EVENT, YAML_SCALAR_TOKEN, YAML_SEQUENCE_END_EVENT, YAML_SEQUENCE_START_EVENT, YAML_STREAM_END_EVENT,
    YAML_STREAM_END_TOKEN, YAML_STREAM_START_EVENT, YAML_STREAM_START_TOKEN, YAML_TAG_DIRECTIVE_TOKEN, YAML_TAG_TOKEN,
...
pub unsafe fn yaml_parser_parse(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
    ...
    memset(event as *mut libc::c_void, 0, size_of::<yaml_event_t>() as libc::c_ulong);
...
unsafe fn yaml_parser_state_machine(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
    match (*parser).state {
        YAML_PARSE_STREAM_START_STATE => yaml_parser_parse_stream_start(parser, event),
        YAML_PARSE_IMPLICIT_DOCUMENT_START_STATE => yaml_parser_parse_document_start(parser, event, true),
        ...
        YAML_PARSE_BLOCK_NODE_OR_INDENTLESS_SEQUENCE_STATE => yaml_parser_parse_node(parser, event, true, true),
        YAML_PARSE_FLOW_NODE_STATE => yaml_parser_parse_node(parser, event, false, false),
        YAML_PARSE_BLOCK_SEQUENCE_FIRST_ENTRY_STATE => yaml_parser_parse_block_sequence_entry(parser, event, true),
        YAML_PARSE_BLOCK_SEQUENCE_ENTRY_STATE => yaml_parser_parse_block_sequence_entry(parser, event, false),
        YAML_PARSE_INDENTLESS_SEQUENCE_ENTRY_STATE => yaml_parser_parse_indentless_sequence_entry(parser, event),
        YAML_PARSE_BLOCK_MAPPING_FIRST_KEY_STATE => yaml_parser_parse_block_mapping_key(parser, event, true),
        YAML_PARSE_BLOCK_MAPPING_KEY_STATE => yaml_parser_parse_block_mapping_key(parser, event, false),
        YAML_PARSE_BLOCK_MAPPING_VALUE_STATE => yaml_parser_parse_block_mapping_value(parser, event),
        YAML_PARSE_FLOW_SEQUENCE_FIRST_ENTRY_STATE => yaml_parser_parse_flow_sequence_entry(parser, event, true),
        YAML_PARSE_FLOW_SEQUENCE_ENTRY_STATE => yaml_parser_parse_flow_sequence_entry(parser, event, false),
        ...
        YAML_PARSE_FLOW_MAPPING_FIRST_KEY_STATE => yaml_parser_parse_flow_mapping_key(parser, event, true),
        YAML_PARSE_FLOW_MAPPING_KEY_STATE => yaml_parser_parse_flow_mapping_key(parser, event, false),
        YAML_PARSE_FLOW_MAPPING_VALUE_STATE => yaml_parser_parse_flow_mapping_value(parser, event, false),
        YAML_PARSE_FLOW_MAPPING_EMPTY_VALUE_STATE => yaml_parser_parse_flow_mapping_value(parser, event, true),
        _ => FAIL,
    }
}

unsafe fn yaml_parser_parse_stream_start(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
...
unsafe fn yaml_parser_parse_document_start(
    ...
    let mut version_directive: *mut yaml_version_directive_t = ptr::null_mut::<yaml_version_directive_t>();
...
unsafe fn yaml_parser_parse_document_content(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
    let token: *mut yaml_token_t = PEEK_TOKEN(parser);
    if token.is_null() {
        return FAIL;
    }
...
unsafe fn yaml_parser_parse_document_end(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
            ...
            if !tag_handle.is_null() {
                if *tag_handle == 0 {
                    ...
                    loop {
                        ...
                        if strcmp((*tag_directive).handle as *mut libc::c_char, tag_handle as *mut libc::c_char) == 0 {
                            ...
                            tag = yaml_malloc(prefix_len.force_add(suffix_len).force_add(1_u64)) as *mut yaml_char_t;
                            memcpy(
                ...
                if indentless_sequence && (*token).type_ == YAML_BLOCK_ENTRY_TOKEN {
                    ...
                    *fresh38 = tag;
                    ...
                    if (*token).data.scalar.style == YAML_PLAIN_SCALAR_STYLE && tag.is_null()
                        || !tag.is_null()
                            && strcmp(tag as *mut libc::c_char, b"!\0" as *const u8 as *const libc::c_char) == 0
                    {
                        plain_implicit = true;
                    } else if tag.is_null() {
...
unsafe fn yaml_parser_parse_indentless_sequence_entry(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
    let mut token: *mut yaml_token_t;
    token = PEEK_TOKEN(parser);
    if token.is_null() {
        return FAIL;
...
unsafe fn yaml_parser_parse_block_mapping_value(parser: *mut yaml_parser_t, event: *mut yaml_event_t) -> Success {
    let mut token: *mut yaml_token_t;
    token = PEEK_TOKEN(parser);
    if token.is_null() {
        return FAIL;
...
unsafe fn yaml_parser_parse_flow_sequence_entry_mapping_key(
    ...
    {
        PUSH!((*parser).states, YAML_PARSE_FLOW_SEQUENCE_ENTRY_MAPPING_VALUE_STATE);
...
unsafe fn yaml_parser_parse_flow_sequence_entry_mapping_value(
    ...
    if (*token).type_ == YAML_VALUE_TOKEN {
        ...
        if (*token).type_ != YAML_FLOW_ENTRY_TOKEN && (*token).type_ != YAML_FLOW_SEQUENCE_END_TOKEN {
            PUSH!((*parser).states, YAML_PARSE_FLOW_SEQUENCE_ENTRY_MAPPING_END_STATE);
            return yaml_parser_parse_node(parser, event, false, false);
        }
    }
...
unsafe fn yaml_parser_process_directives(
    ...
    if !token.is_null() {
        loop {
            if !((*token).type_ == YAML_VERSION_DIRECTIVE_TOKEN || (*token).type_ == YAML_TAG_DIRECTIVE_TOKEN) {
            ...
            if (*token).type_ == YAML_VERSION_DIRECTIVE_TOKEN {
                if !version_directive.is_null() {
                    ...
                    || (*token).data.version_directive.minor != 1 && (*token).data.version_directive.minor != 2
                {
                    yaml_parser_set_parser_error(
                        parser,
                    ...
                    version_directive = yaml_malloc(size_of::<yaml_version_directive_t>() as libc::c_ulong)
        ...
        if current_block != 17143798186130252483 {
            ...
            loop {
                ...
                if yaml_parser_append_tag_directive(parser, *default_tag_directive, true, (*token).start_mark).fail {
                    current_block = 17143798186130252483;
                    break;
                }
