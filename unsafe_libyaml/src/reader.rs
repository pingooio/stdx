...
unsafe fn yaml_parser_determine_encoding(parser: *mut yaml_parser_t) -> Success {
    while !(*parser).eof
        && ((*parser).raw_buffer.last.c_offset_from((*parser).raw_buffer.pointer) as libc::c_long) < 3_i64
    {
        if yaml_parser_update_raw_buffer(parser).fail {
            return FAIL;
        }
    }
    if (*parser).raw_buffer.last.c_offset_from((*parser).raw_buffer.pointer) as libc::c_long >= 2_i64
        && memcmp(
    ...
    } else if (*parser).raw_buffer.last.c_offset_from((*parser).raw_buffer.pointer) as libc::c_long >= 2_i64
        && memcmp(
            (*parser).raw_buffer.pointer as *const libc::c_void,
    ...
    } else if (*parser).raw_buffer.last.c_offset_from((*parser).raw_buffer.pointer) as libc::c_long >= 3_i64
        && memcmp(
            (*parser).raw_buffer.pointer as *const libc::c_void,
...
unsafe fn yaml_parser_update_raw_buffer(parser: *mut yaml_parser_t) -> Success {
        ...
        && (*parser).raw_buffer.pointer < (*parser).raw_buffer.last
    {
        memmove(
            ...
            (*parser).raw_buffer.pointer as *const libc::c_void,
            (*parser).raw_buffer.last.c_offset_from((*parser).raw_buffer.pointer) as libc::c_long as libc::c_ulong,
        );
    }
    let fresh7 = addr_of_mut!((*parser).raw_buffer.last);
    *fresh7 = (*fresh7).wrapping_offset(
        -((*parser).raw_buffer.pointer.c_offset_from((*parser).raw_buffer.start) as libc::c_long as isize),
    );
...
pub(crate) unsafe fn yaml_parser_update_buffer(parser: *mut yaml_parser_t, length: size_t) -> Success {
    let mut first = true;
    ...
    while (*parser).unread < length {
        ...
        while (*parser).raw_buffer.pointer != (*parser).raw_buffer.last {
            ...
            let high: libc::c_int;
            let mut k: size_t;
            let raw_unread: size_t = (*parser).raw_buffer.last.c_offset_from((*parser).raw_buffer.pointer) as size_t;
                ...
                YAML_UTF8_ENCODING => {
                    ...
                    if width as libc::c_ulong > raw_unread {
                        if (*parser).eof {
                            return yaml_parser_set_reader_error(
                                parser,
                                b"incomplete UTF-8 octet sequence\0" as *const u8 as *const libc::c_char,
                                (*parser).offset,
                                -1,
                            );
                        }
                        ...
                        while k < width as libc::c_ulong {
                            ...
                            if octet & 0xC0 != 0x80 {
                                return yaml_parser_set_reader_error(
                                    parser,
                                    b"invalid trailing UTF-8 octet\0" as *const u8 as *const libc::c_char,
                            ...
                            || width == 4 && value >= 0x10000)
                        {
                            return yaml_parser_set_reader_error(
                                parser,
                                b"invalid length of a UTF-8 sequence\0" as *const u8 as *const libc::c_char,
                                (*parser).offset,
                                -1,
                            );
                        }
                ...
                YAML_UTF16LE_ENCODING | YAML_UTF16BE_ENCODING => {
                    ...
                    if raw_unread < 2_u64 {
                        if (*parser).eof {
                            return yaml_parser_set_reader_error(
                                parser,
                                b"incomplete UTF-16 character\0" as *const u8 as *const libc::c_char,
                                (*parser).offset,
                                -1,
                            );
                        }
                        ...
                        value = (*(*parser).raw_buffer.pointer.wrapping_offset(low as isize) as libc::c_int
                            + ((*(*parser).raw_buffer.pointer.wrapping_offset(high as isize) as libc::c_int) << 8))
                            as libc::c_uint;
                        if value & 0xFC00 == 0xDC00 {
                            return yaml_parser_set_reader_error(
                                parser,
                                b"unexpected low surrogate area\0" as *const u8 as *const libc::c_char,
                                (*parser).offset,
                                value as libc::c_int,
                            );
                        }
                        if value & 0xFC00 == 0xD800 {
                            ...
                            if raw_unread < 4_u64 {
                                if (*parser).eof {
                                    return yaml_parser_set_reader_error(
                                        parser,
                                        b"incomplete UTF-16 surrogate pair\0" as *const u8 as *const libc::c_char,
                                        (*parser).offset,
                                        -1,
                                    );
                                }
                                ...
                                value2 = (*(*parser).raw_buffer.pointer.wrapping_offset((low + 2) as isize)
                                    as libc::c_int
                                    + ((*(*parser).raw_buffer.pointer.wrapping_offset((high + 2) as isize)
                                        as libc::c_int)
                                        << 8)) as libc::c_uint;
                                if value2 & 0xFC00 != 0xDC00 {
                                    return yaml_parser_set_reader_error(
                                        parser,
                                        b"expected low surrogate area\0" as *const u8 as *const libc::c_char,
                                        (*parser).offset.force_add(2_u64),
                                        value2 as libc::c_int,
                                    );
                                }
                                value = 0x10000_u32.force_add((value & 0x3FF) << 10).force_add(value2 & 0x3FF);
                            }
                        } else {
                            width = 2;
                        }
