...
use crate::{
    api::{yaml_free, yaml_malloc, yaml_queue_extend, yaml_stack_extend, yaml_string_extend, yaml_string_join},
    externs::{memcpy, memmove, memset, strcmp, strlen},
    libc,
    ops::{ForceAdd as _, ForceMul as _},
    reader::yaml_parser_update_buffer,
    success::{Success, FAIL, OK},
    yaml::{ptrdiff_t, size_t, yaml_char_t, yaml_string_t, NULL_STRING},
    yaml_mark_t, yaml_parser_t, yaml_simple_key_t, yaml_token_t, yaml_token_type_t, PointerExt, YAML_ALIAS_TOKEN,
    YAML_ANCHOR_TOKEN, YAML_BLOCK_END_TOKEN, YAML_BLOCK_ENTRY_TOKEN, YAML_BLOCK_MAPPING_START_TOKEN,
    YAML_BLOCK_SEQUENCE_START_TOKEN, YAML_DOCUMENT_END_TOKEN, YAML_DOCUMENT_START_TOKEN,
    YAML_DOUBLE_QUOTED_SCALAR_STYLE, YAML_FLOW_ENTRY_TOKEN, YAML_FLOW_MAPPING_END_TOKEN, YAML_FLOW_MAPPING_START_TOKEN,
    YAML_FLOW_SEQUENCE_END_TOKEN, YAML_FLOW_SEQUENCE_START_TOKEN, YAML_FOLDED_SCALAR_STYLE, YAML_KEY_TOKEN,
    YAML_LITERAL_SCALAR_STYLE, YAML_MEMORY_ERROR, YAML_NO_ERROR, YAML_PLAIN_SCALAR_STYLE, YAML_SCALAR_TOKEN,
    YAML_SCANNER_ERROR, YAML_SINGLE_QUOTED_SCALAR_STYLE, YAML_STREAM_END_TOKEN, YAML_STREAM_START_TOKEN,
...
pub unsafe fn yaml_parser_scan(parser: *mut yaml_parser_t, token: *mut yaml_token_t) -> Success {
    ...
    memset(token as *mut libc::c_void, 0, size_of::<yaml_token_t>() as libc::c_ulong);
...
unsafe fn yaml_parser_fetch_next_token(parser: *mut yaml_parser_t) -> Success {
    ...
    if CHECK!((*parser).buffer, b'?') && ((*parser).flow_level != 0 || IS_BLANKZ_AT!((*parser).buffer, 1)) {
...
unsafe fn yaml_parser_stale_simple_keys(parser: *mut yaml_parser_t) -> Success {
    ...
    while simple_key != (*parser).simple_keys.top {
                ...
                || (*simple_key).mark.index.force_add(1024_u64) < (*parser).mark.index)
        {
            ...
            if (*parser).simple_keys.start.add((*parser).not_simple_keys as usize) == simple_key {
...
unsafe fn yaml_parser_save_simple_key(parser: *mut yaml_parser_t) -> Success {
    let required = (*parser).flow_level == 0 && (*parser).indent as libc::c_long == (*parser).mark.column as ptrdiff_t;
    if (*parser).simple_key_allowed {
        ...
        *(*parser).simple_keys.top.wrapping_offset(-1_isize) = simple_key;
        if (*parser).simple_keys.start.add((*parser).not_simple_keys as usize) == (*parser).simple_keys.top {
            (*parser).not_simple_keys -= 1;
        }
    }
    OK
...
unsafe fn yaml_parser_fetch_document_indicator(parser: *mut yaml_parser_t, type_: yaml_token_type_t) -> Success {
    let mut token = MaybeUninit::<yaml_token_t>::uninit();
    let token = token.as_mut_ptr();
    yaml_parser_unroll_indent(parser, -1_i64);
    if yaml_parser_remove_simple_key(parser).fail {
...
unsafe fn yaml_parser_fetch_flow_collection_start(parser: *mut yaml_parser_t, type_: yaml_token_type_t) -> Success {
    let mut token = MaybeUninit::<yaml_token_t>::uninit();
    let token = token.as_mut_ptr();
...
unsafe fn yaml_parser_fetch_flow_collection_end(parser: *mut yaml_parser_t, type_: yaml_token_type_t) -> Success {
    let mut token = MaybeUninit::<yaml_token_t>::uninit();
    let token = token.as_mut_ptr();
    if yaml_parser_remove_simple_key(parser).fail {
        return FAIL;
    ...
    if (*parser).flow_level == 0 {
        if !(*parser).simple_key_allowed {
            yaml_parser_set_scanner_error(
                ...
                (*parser).mark,
                b"block sequence entries are not allowed in this context\0" as *const u8 as *const libc::c_char,
            );
            return FAIL;
        }
    ...
    if (*parser).flow_level == 0 {
        if !(*parser).simple_key_allowed {
            yaml_parser_set_scanner_error(
                ...
                ptr::null::<libc::c_char>(),
                (*parser).mark,
                b"mapping keys are not allowed in this context\0" as *const u8 as *const libc::c_char,
            );
            return FAIL;
        }
        if yaml_parser_roll_indent(
        ...
        if (*parser).flow_level == 0 {
            if !(*parser).simple_key_allowed {
                yaml_parser_set_scanner_error(
                    ...
                    ptr::null::<libc::c_char>(),
                    (*parser).mark,
                    b"mapping values are not allowed in this context\0" as *const u8 as *const libc::c_char,
                );
                return FAIL;
            }
            if yaml_parser_roll_indent(
...
unsafe fn yaml_parser_fetch_anchor(parser: *mut yaml_parser_t, type_: yaml_token_type_t) -> Success {
    let mut token = MaybeUninit::<yaml_token_t>::uninit();
    let token = token.as_mut_ptr();
    if yaml_parser_save_simple_key(parser).fail {
        return FAIL;
...
unsafe fn yaml_parser_scan_to_next_token(parser: *mut yaml_parser_t) -> Success {
    loop {
        ...
        while CHECK!((*parser).buffer, b' ')
            || ((*parser).flow_level != 0 || !(*parser).simple_key_allowed) && CHECK!((*parser).buffer, b'\t')
        {
            SKIP(parser);
            if CACHE(parser, 1_u64).fail {
                return FAIL;
...
unsafe fn yaml_parser_scan_directive(parser: *mut yaml_parser_t, token: *mut yaml_token_t) -> Success {
    ...
    if yaml_parser_scan_directive_name(parser, start_mark, addr_of_mut!(name)).ok {
        if strcmp(name as *mut libc::c_char, b"YAML\0" as *const u8 as *const libc::c_char) == 0 {
            if yaml_parser_scan_version_directive_value(parser, start_mark, addr_of_mut!(major), addr_of_mut!(minor))
                .fail
            {
        ...
        } else if strcmp(name as *mut libc::c_char, b"TAG\0" as *const u8 as *const libc::c_char) == 0 {
            if yaml_parser_scan_tag_directive_value(parser, start_mark, addr_of_mut!(handle), addr_of_mut!(prefix)).fail
            {
                current_block = 11397968426844348457;
            } else {
                end_mark = (*parser).mark;
                    ...
                    if current_block != 11397968426844348457 {
                        if !IS_BREAKZ!((*parser).buffer) {
                            yaml_parser_set_scanner_error(
                                ...
                                b"while scanning a directive\0" as *const u8 as *const libc::c_char,
                                start_mark,
                                b"did not find expected comment or line break\0" as *const u8 as *const libc::c_char,
                            );
                        } else {
                            if IS_BREAK!((*parser).buffer) {
                                if CACHE(parser, 2_u64).fail {
        ...
        match current_block {
            ...
            _ => {
                if IS_BLANK!((*parser).buffer) {
                    ...
                    if yaml_parser_scan_tag_handle(parser, true, start_mark, addr_of_mut!(handle_value)).fail {
                        current_block = 5231181710497607163;
                        continue;
                    }
                    if CACHE(parser, 1_u64).fail {
                    ...
                    if !IS_BLANK!((*parser).buffer) {
                        yaml_parser_set_scanner_error(
                            ...
                            start_mark,
                            b"did not find expected whitespace\0" as *const u8 as *const libc::c_char,
                        );
                        current_block = 5231181710497607163;
                    } else {
                        while IS_BLANK!((*parser).buffer) {
        ...
        if current_block != 5883759901342942623 {
                    ...
                    || CHECK!((*parser).buffer, b'`'))
            {
                yaml_parser_set_scanner_error(
                    ...
                    start_mark,
                    b"did not find expected alphabetic or numeric character\0" as *const u8 as *const libc::c_char,
                );
            } else {
                if type_ == YAML_ANCHOR_TOKEN {
                    memset(token as *mut libc::c_void, 0, size_of::<yaml_token_t>() as libc::c_ulong);
    ...
    if CACHE(parser, 2_u64).ok {
        if CHECK_AT!((*parser).buffer, b'<', 1) {
            ...
            *handle = b'\0';
            ...
            && *handle.wrapping_offset(strlen(handle as *mut libc::c_char).wrapping_sub(1_u64) as isize) == b'!'
        {
            if yaml_parser_scan_tag_uri(
                parser,
                false,
        ...
        } else if yaml_parser_scan_tag_uri(parser, false, false, handle, start_mark, addr_of_mut!(suffix)).fail {
            current_block = 17708497480799081542;
        } else {
            yaml_free(handle as *mut libc::c_void);
        ...
        if !CHECK!((*parser).buffer, b'!') {
            ...
            if CACHE(parser, 1_u64).ok {
                ...
                if current_block != 1771849829115608806 {
                    ...
                    } else if directive && !(*string.start == b'!' && *string.start.wrapping_offset(1_isize) == b'\0') {
                        yaml_parser_set_scanner_error(
                            parser,
            ...
            _ => {
                if string.end.c_offset_from(string.start) as size_t <= length {
                    ...
                    if length > 1_u64 {
                        ...
                        string.pointer = string.pointer.wrapping_offset(length.wrapping_sub(1_u64) as isize);
                    }
                    if CACHE(parser, 1_u64).fail {
                                ...
                                || CHECK!((*parser).buffer, b']'))
                    {
                        if CHECK!((*parser).buffer, b'%') {
                            STRING_EXTEND!(string);
                            if yaml_parser_scan_uri_escapes(parser, directive, start_mark, addr_of_mut!(string)).fail {
                                current_block = 15265153392498847348;
                                continue 'c_21953;
                            }
                        } else {
...
unsafe fn yaml_parser_scan_uri_escapes(
    ...
    loop {
        ...
        if !(CHECK!((*parser).buffer, b'%') && IS_HEX_AT!((*parser).buffer, 1) && IS_HEX_AT!((*parser).buffer, 2)) {
            yaml_parser_set_scanner_error(
                parser,
                if directive {
                    b"while parsing a %TAG directive\0" as *const u8 as *const libc::c_char
            ...
            if CACHE(parser, 1_u64).fail {
                ...
                if CHECK!((*parser).buffer, b'0') {
                    yaml_parser_set_scanner_error(
                        ...
                        start_mark,
                        b"found an indentation indicator equal to 0\0" as *const u8 as *const libc::c_char,
                    );
                    current_block = 14984465786483313892;
                } else {
...
unsafe fn yaml_parser_scan_block_scalar_breaks(
    ...
    *end_mark = (*parser).mark;
    loop {
        ...
        while (*indent == 0 || ((*parser).mark.column as libc::c_int) < *indent) && IS_SPACE!((*parser).buffer) {
            SKIP(parser);
            if CACHE(parser, 1_u64).fail {
                return FAIL;
            }
        ...
        if (*indent == 0 || ((*parser).mark.column as libc::c_int) < *indent) && IS_TAB!((*parser).buffer) {
            yaml_parser_set_scanner_error(
                ...
                b"while scanning a block scalar\0" as *const u8 as *const libc::c_char,
                start_mark,
                b"found a tab character where an indentation space is expected\0" as *const u8 as *const libc::c_char,
            );
            return FAIL;
        }
        if !IS_BREAK!((*parser).buffer) {
...
unsafe fn yaml_parser_scan_flow_scalar(parser: *mut yaml_parser_t, token: *mut yaml_token_t, single: bool) -> Success {
    let current_block: u64;
    let end_mark: yaml_mark_t;
    let mut string = NULL_STRING;
    let mut leading_break = NULL_STRING;
    ...
    's_58: loop {
            ...
            && IS_BLANKZ_AT!((*parser).buffer, 3)
        {
            ...
            while !IS_BLANKZ!((*parser).buffer) {
                if single && CHECK_AT!((*parser).buffer, b'\'', 0) && CHECK_AT!((*parser).buffer, b'\'', 1) {
                    STRING_EXTEND!(string);
                    ...
                    *fresh521 = b'\'';
                    ...
                    if !single && CHECK!((*parser).buffer, b'\\') && IS_BREAK_AT!((*parser).buffer, 1) {
                        if CACHE(parser, 3_u64).fail {
                            current_block = 8114179180390253173;
                            break 's_58;
                        }
                        ...
                        match *(*parser).buffer.pointer.wrapping_offset(1_isize) {
                            ...
                            _ => {
                                yaml_parser_set_scanner_error(
                                    parser,
                                    b"while parsing a quoted scalar\0" as *const u8 as *const libc::c_char,
                                    start_mark,
                                    b"found unknown escape character\0" as *const u8 as *const libc::c_char,
                                );
                                current_block = 8114179180390253173;
                                break 's_58;
                            }
                        ...
                        if code_length != 0 {
                            ...
                            while k < code_length {
                                if !IS_HEX_AT!((*parser).buffer, k as isize) {
                                    ...
                                    value = (value << 4)
                                        .force_add(AS_HEX_AT!((*parser).buffer, k as isize) as libc::c_uint);
                                    k = k.force_add(1);
                                }
                            }
                            if value >= 0xD800 && value <= 0xDFFF || value > 0x10FFFF {
                                ...
                                if value <= 0x7F {
                                    ...
                                    *fresh576 = 0xE0_u32.force_add(value >> 12) as yaml_char_t;
                                    ...
                                    string.pointer = string.pointer.wrapping_offset(1);
                                    *fresh577 = 0x80_u32.force_add(value >> 6 & 0x3F) as yaml_char_t;
                                    ...
                                    *fresh579 = 0xF0_u32.force_add(value >> 18) as yaml_char_t;
                                    ...
                                    string.pointer = string.pointer.wrapping_offset(1);
                                    *fresh580 = 0x80_u32.force_add(value >> 12 & 0x3F) as yaml_char_t;
                                    let fresh581 = string.pointer;
                                    string.pointer = string.pointer.wrapping_offset(1);
                                    *fresh581 = 0x80_u32.force_add(value >> 6 & 0x3F) as yaml_char_t;
...
unsafe fn yaml_parser_scan_plain_scalar(parser: *mut yaml_parser_t, token: *mut yaml_token_t) -> Success {
    let current_block: u64;
    let mut end_mark: yaml_mark_t;
    let mut string = NULL_STRING;
    let mut leading_break = NULL_STRING;
    ...
    's_57: loop {
        ...
        while IS_BLANK!((*parser).buffer) || IS_BREAK!((*parser).buffer) {
            if IS_BLANK!((*parser).buffer) {
                if leading_blanks && ((*parser).mark.column as libc::c_int) < indent && IS_TAB!((*parser).buffer) {
                    yaml_parser_set_scanner_error(
                        parser,
                        b"while scanning a plain scalar\0" as *const u8 as *const libc::c_char,
                        start_mark,
                        b"found a tab character that violates indentation\0" as *const u8 as *const libc::c_char,
                    );
                    current_block = 16642808987012640029;
                    break 's_57;
