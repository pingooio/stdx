...
pub unsafe fn yaml_emitter_flush(emitter: *mut yaml_emitter_t) -> Success {
    ...
    *fresh2 = (*emitter).buffer.start;
    ...
    if (*emitter).encoding == YAML_UTF8_ENCODING {
        ...
        ) != 0
        {
            ...
            *fresh4 = (*emitter).buffer.start;
            ...
            return yaml_emitter_set_writer_error(emitter, b"write error\0" as *const u8 as *const libc::c_char);
        }
    }
    ...
    while (*emitter).buffer.pointer != (*emitter).buffer.last {
        ...
        *fresh5 = (*fresh5).wrapping_offset(width as isize);
        if value < 0x10000 {
            ...
            *(*emitter).raw_buffer.last.wrapping_offset(low as isize) = (value & 0xFF) as libc::c_uchar;
            ...
            *(*emitter).raw_buffer.last.wrapping_offset(low as isize) = (value >> 10 & 0xFF) as libc::c_uchar;
            *(*emitter).raw_buffer.last.wrapping_offset((high + 2) as isize) =
                0xDC_u32.force_add(value >> 8 & 0xFF) as libc::c_uchar;
            *(*emitter).raw_buffer.last.wrapping_offset((low + 2) as isize) = (value & 0xFF) as libc::c_uchar;
    ...
    if (*emitter).write_handler.expect("non-null function pointer")(
        ...
        (*emitter).raw_buffer.last.c_offset_from((*emitter).raw_buffer.start) as size_t,
    ) != 0
    {
        ...
        *fresh11 = (*emitter).raw_buffer.start;
        ...
        yaml_emitter_set_writer_error(emitter, b"write error\0" as *const u8 as *const libc::c_char)
    }
}
