...
func TestEqual(t *testing.T) {
	...
	for range 100 {
		uuid := NewV4()
		if uuid.Equal(uuid) == false {
			t.Fatalf("%s should be equal to %s", uuid.String(), uuid.String())
		}

		if uuid.Equal(zero) {
			t.Fatalf("%s should be different than %s", uuid.String(), zero.String())
		}

		if uuid.Equal(someUuid) {
			t.Fatalf("%s should be different than %s", uuid.String(), someUuid.String())
		}
	}
